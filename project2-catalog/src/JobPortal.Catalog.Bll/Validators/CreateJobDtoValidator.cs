...
public class CreateJobDtoValidator : AbstractValidator<CreateJobDto>
{
    public CreateJobDtoValidator()
    {
        ...
        RuleFor(x => x.EmploymentType)
            ...
            .Must(BeAValidEmploymentType).WithMessage("Employment type must be one of: Full-time, Part-time, Contract, Internship");

        RuleFor(x => x.ExperienceYears)
            .GreaterThanOrEqualTo(0).WithMessage("Experience years must be greater than or equal to 0");
    }

    private bool BeAValidEmploymentType(string employmentType)
    {
        var validTypes = new[] { "Full-time", "Part-time", "Contract", "Internship" };
        return validTypes.Contains(employmentType);
    }
}
